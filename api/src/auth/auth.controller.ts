...
export class AuthController {
    ...
    @Post('sign-in')
    @Public()
    async signIn(@Body() signInDto: SignInDto, @Res({passthrough: true}) response): Promise<{ lastSignedIn: Date }> {
        const authData = await this.authService.signIn(signInDto.email, signInDto.password);
    ...
    async refreshToken(@Req() request, @Res({passthrough: true}) response) {
        const res = await this.authService.refreshToken(request.cookies?.refreshToken);
