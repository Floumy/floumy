...
export class GitlabService {
    ...
    private async processMergeRequests(project: Project) {
        ...
        for (const mergeRequest of mergeRequests) {
            const workItem = await this.getMergeRequestWorkItem(project, mergeRequest);

            const notes = await gitlab.MergeRequestNotes.all(project.gitlabProjectId, mergeRequest.iid)

            const approvedAt = notes.find((note) => note.body.includes('approved this merge request'))?.created_at;
            ...
            if (notes && notes.length > 0) {
                ...
                firstReviewAt = firstReviewNote ? new Date(firstReviewNote.created_at) : null;
            }
    ...
    private async handleNewMergeRequest(
        ...
        const workItem = await this.getMergeRequestWorkItem(project, mergeRequestEvent.object_attributes);
