...
export default function CreateRequest({ onSubmit }) {
  ...
  const handleSubmit = async (values) => {
    try {
      setIsLoading(true);
      const savedRequest = await onSubmit(values);
      setTimeout(() => toast.success('The request has been saved'), 100);
      navigate(getDetailUrl(savedRequest.id), { replace: true });
    } catch (e) {
      toast.error('The request could not be saved');
    } finally {
