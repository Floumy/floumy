...
import { listPublicCyclesWithWorkItemsForTimeline } from '../../../services/cycles/cycles.service';
import {
  formatDate,
  formatTimeline,
  getCycleEndDate,
...
function PublicSprints() {
  ...
  useEffect(() => {
    ...
    async function fetchSprints() {
      ...
      try {
        const sprints = await listPublicCyclesWithWorkItemsForTimeline(
          orgId,
          projectId,
          timelineFilterValue,
        );
                      ...
                      <Row className="pl-4 pt-2 pr-4">
                        <Col>
                          <h3 className="mb-0">
                            ...
                            <Link
                              to={`/public/orgs/${orgId}/projects/${projectId}/cycles/detail/${sprint.id}`}
                              className="mr-2"
                            >
                              <span className="text-muted">
                                {formatDate(getCycleStartDate(sprint))} -{' '}
                                {formatDate(getCycleEndDate(sprint))}
                              </span>{' '}
                              | {sprint.title}
                            </Link>
