...
import {
  getCurrentUserMcpToken,
  refreshCurrentUserMcpToken,
...
export default function AiSettings() {
  ...
  const refreshToken = async () => {
    try {
      setIsLoading(true);
      const newToken = await refreshCurrentUserMcpToken();
      setMcpToken(newToken);
      toast.success('MCP token refreshed successfully');
    } catch (error) {
      toast.error('Failed to refresh MCP token');
    } finally {
      setIsLoading(false);
    }
  };
                          ...
                          <div className="pl-4 border-left border-light">
                            <ol className="pl-3">
                              ...
                              <li className="mb-3">
                                ...
                                app:
                                <div className="bg-lighter p-3 rounded mt-2 mb-2 code-block">
                                  <pre className="mb-0">{`{
  "mcpServers": {
    "floumy": {
      "command": "npx",
