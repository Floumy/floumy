...
export function getWorkItemUpdates(item) {
  ...
  if (current.status !== previous.status) {
    updates.push(
      `Status changed from "${formatWorkItemStatusName(previous.status)}" to "${formatWorkItemStatusName(current.status)}"`,
    );
  }

  if (current.type !== previous.type) {
    updates.push(
      `Type changed from "${formatWorkItemTypeName(previous.type)}" to "${formatWorkItemTypeName(current.type)}"`,
    );
  }

  if (current.priority !== previous.priority) {
    updates.push(
      `Priority changed from "${priorityName(previous.priority)}" to "${priorityName(current.priority)}"`,
    );
  }

  if (current.estimation !== previous.estimation) {
    updates.push(
      `Estimation changed from ${previous?.estimation || 'None'} to ${current?.estimation || 'None'}`,
    );
  }
  ...
  if (current?.sprint?.id !== previous?.sprint?.id) {
    updates.push(`Cycle changed to "${current?.sprint?.title || 'None'}"`);
  }

  if (current?.initiative?.id !== previous?.initiative?.id) {
    updates.push(
      `Initiative changed to "${current?.initiative?.title || 'None'}"`,
    );
  }
...
export function getOkrUpdates(item) {
  ...
  if (current.objective.title !== previous.objective.title) {
    updates.push(
      `Title changed from "${previous.objective.title}" to "${current.objective.title}"`,
    );
  }

  if (current.objective.status !== previous.objective.status) {
    updates.push(
      `Status changed from "${formatHyphenatedString(previous.objective.status)}" to "${formatHyphenatedString(current.objective.status)}"`,
    );
  }

  if (current.objective.progress !== previous.objective.progress) {
    updates.push(
      `Progress changed from ${formatOKRsProgress(previous.objective.progress)}% to ${formatOKRsProgress(current.objective.progress)}%`,
    );
  }

  if (current.objective.timeline !== previous.objective.timeline) {
    updates.push(
      `Timeline changed from "${formatTimeline(previous.objective.timeline)}" to "${formatTimeline(current.objective.timeline)}"`,
    );
  }
  ...
  if (current.objective.assignedTo !== previous.objective.assignedTo) {
    updates.push(
      `Asignee changed to "${current.objective.assignedTo?.name || 'None'}"`,
    );
  }
...
export function getKeyResultUpdates(item) {
  ...
  if (current.status !== previous.status) {
    updates.push(
      `Status changed from "${formatHyphenatedString(previous.status)}" to "${formatHyphenatedString(current.status)}"`,
    );
  }

  if (current.progress !== previous.progress) {
    updates.push(
      `Progress changed from ${formatOKRsProgress(previous.progress)}% to ${formatOKRsProgress(current.progress)}%`,
    );
  }

  if (current.timeline !== previous.timeline) {
    updates.push(
      `Timeline changed from "${formatTimeline(previous.timeline)}" to "${formatTimeline(current.timeline)}"`,
    );
  }
...
export function getInitiativeUpdates(item) {
  ...
  if (current.keyResult?.id !== previous.keyResult?.id) {
    updates.push(
      `Key result changed to "${current.keyResult?.title || 'None'}"`,
    );
  }

  if (current.milestone?.id !== previous.milestone?.id) {
    updates.push(
      `Milestone changed to "${current.milestone?.title || 'None'}"`,
    );
  }
