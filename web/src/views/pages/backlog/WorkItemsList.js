...
function WorkItemsList({
  ...
  function handleContextMenu(event, workItem) {
    ...
    if (!contextMenuSelectedWorkItems.includes(workItem.id)) {
      ...
      setLastSelectedWorkItem([
        workItems.findIndex((wi) => wi.id === workItem.id),
      ]);
    }

    const contextMenuWorkItems =
      contextMenuSelectedWorkItems.length > 0
        ? workItems.filter((workItem) =>
            contextMenuSelectedWorkItems.includes(workItem.id),
          )
        : [workItem];
    show({
      event,
      props: { workItems: contextMenuWorkItems },
    });
  }
  ...
  const handleRowClick = (workItemId, index, event) => {
    ...
    if (event.shiftKey && lastSelectedWorkItem !== null) {
      const [startIndex, endIndex] = [
        Math.min(index, lastSelectedWorkItem),
        Math.max(index, lastSelectedWorkItem),
      ];
      const rowsToToggle = workItems
        .slice(startIndex, endIndex + 1)
        .map((row) => row.id);

      const shouldSelect = !selectedWorkItems.includes(workItemId);
      newSelectedWorkItems = selectedWorkItems.filter(
        (id) => !rowsToToggle.includes(id),
      );

      if (shouldSelect) {
        newSelectedWorkItems = [
          ...new Set([...newSelectedWorkItems, ...rowsToToggle]),
        ];
      }
    } else {
          ...
          <tbody className="list">
            ...
            {workItems.map((workItem, index) => (
              <tr
                ...
                className={
                  selectedWorkItems.includes(workItem.id) ? 'selected-row' : ''
                }
              >
                <td>
                  <Link
