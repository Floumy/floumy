...
function CreateUpdateDeleteWorkItem({ onSubmit, workItem = defaultWorkItem }) {
  ...
  const loadAndSetInitiatives = useCallback(async () => {
    ...
    const mappedInitiatives = initiatives.map((initiative) => {
      return {
        id: initiative.id,
        text: `${initiative.reference}: ${initiative.title}`,
      };
    });
  ...
  async function handleSubmit(values) {
    try {
      ...
      if (!workItem.id) {
        navigate(
          `/admin/orgs/${orgId}/projects/${projectId}/work-item/edit/${savedWorkItem.id}`,
          { replace: true },
        );
      }
    } catch (e) {
            ...
            <CardBody>
              <Row>
                <Col>
                    ...
                    {workItem.branches?.map((branch) => (
                      <li key={branch.id} className="mb-2">
                        {branch.state === 'closed' ? (
                          <>
                            <UncontrolledTooltip
                              target={'closed-branch-' + branch.id}
                              placement="top"
                            >
                              The branch is closed
                            </UncontrolledTooltip>
                            <i
                              className="fa fa-times-circle mr-2 text-danger"
                              id={'closed-branch-' + branch.id}
                            />
                  ...
                  <h4>
                    {workItem.codeConnectionType === 'gitlab'
                      ? 'Merge Requests'
                      : 'Pull Requests'}
                  </h4>
                  <ul className="list-unstyled">
