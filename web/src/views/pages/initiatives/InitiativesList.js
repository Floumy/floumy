...
function InitiativesList({
  ...
  function handleContextMenu(event, initiative) {
    ...
    if (!contextMenuSelectedInitiatives.includes(initiative.id)) {
      ...
      setLastSelectedInitiative([
        initiatives.findIndex((f) => f.id === initiative.id),
      ]);
    }

    const contextMenuInitiatives =
      contextMenuSelectedInitiatives.length > 0
        ? initiatives.filter((initiative) =>
            contextMenuSelectedInitiatives.includes(initiative.id),
          )
        : [initiative];
    show({
      event,
      props: { initiatives: contextMenuInitiatives },
    });
  }
  ...
  const handleRowClick = (initiativeId, index, event) => {
    ...
    if (event.shiftKey && lastSelectedInitiative !== null) {
      const [startIndex, endIndex] = [
        Math.min(index, lastSelectedInitiative),
        Math.max(index, lastSelectedInitiative),
      ];
      const rowsToToggle = initiatives
        .slice(startIndex, endIndex + 1)
        .map((row) => row.id);

      const shouldSelect = !selectedInitiatives.includes(initiativeId);
      newSelectedInitiatives = selectedInitiatives.filter(
        (id) => !rowsToToggle.includes(id),
      );

      if (shouldSelect) {
        newSelectedInitiatives = [
          ...new Set([...newSelectedInitiatives, ...rowsToToggle]),
        ];
      }
    } else {
    ...
    <>
      {enableContextMenu && (
        <InitiativesContextMenu
          ...
          onChangeAssignTo={onChangeAssignedTo}
