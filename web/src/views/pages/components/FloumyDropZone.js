...
import {
  ...
  downloadFile,
  uploadFile,
} from '../../../services/files/uploads.service';
...
export function FloumyDropZone({ onFilesChanged, initialFiles = [] }) {
  ...
  const uploadFileSafely = async (file) => {
    try {
      ...
      setFiles((prevFiles) =>
        prevFiles.map((f) =>
          f.name === file.name ? { ...f, uploading: false, uploadedFile } : f,
        ),
      );
    } catch (e) {
  ...
  const prepareFilesForUploading = (filesToPrepare) => {
    setFiles((prevFiles) => [
      ...prevFiles,
      ...filesToPrepare.map((file) => ({ name: file.name, uploading: true })),
    ]);
  };
  ...
  const removeFile = async (fileId) => {
    try {
      ...
      setFiles((prevFiles) =>
        prevFiles.filter((file) => file.uploadedFile.id !== fileId),
      );
    } catch (e) {
  ...
  useEffect(() => {
    onFilesChanged(
      files.filter((file) => !file.uploading).map((file) => file.uploadedFile),
    );
  ...
  async function downloadUploadedFile(file) {
    if (file.uploadedFile)
      await downloadFile(orgId, projectId, file.uploadedFile.id);
  }
    ...
    <>
      <div className="floumy-drop-zone" {...getRootProps()}>
        <div>
          <input {...getInputProps()} />
          <p>
            <i className="fa fa-upload" /> Drag 'n' drop some files here, or
          ...
          {files.map((file) => (
            <li key={file.name}>
              ...
              >
                ...
                {!file.uploading && (
                  <button
                    className="btn btn-link btn-remove m-0 p-0 mr-2"
                    onClick={async (e) => {
                      e.stopPropagation();
