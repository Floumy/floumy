...
function GitHub() {
  ...
  useEffect(() => {
    ...
    const fetchGithubData = async () => {
      setIsLoading(true);
      try {
        const { connected, repo } = await getIsGithubConnected(
          orgId,
          currentProject.id,
        );
  ...
  const handleRepoUpdate = async () => {
    try {
      const projectRepo = await updateProjectGithubRepo(
        orgId,
        currentProject.id,
        selectedRepo,
      );
    ...
    <>
      ...
      <Container className="mt--6" fluid id="OKRs">
        <UpdateWarning
          ...
          warningMessage={
            'It will take a few minutes for the pull requests on your repository to be processed.'
          }
        ...
        <Row>
          <Col>
            <Card className="mb-5">
                    ...
                    <CardTitle tag="h2" className="mb-3">
                      ...
                      {repo ? (
                        <>
                          Pull Requests{' '}
                          <a
                            className="btn-link text-blue mr-2"
                            href={repo.url}
                            target="_blank"
                            rel="noreferrer"
                          >
                          ...
                          />
                          <UncontrolledTooltip
                            target="disconnect-from-github"
                            placement="top"
                          >
                            Disconnect from GitHub
                          </UncontrolledTooltip>
                          <i
                            className="fa fa-trash-alt mr-2"
                            style={{ cursor: 'pointer' }}
                            id="disconnect-from-github"
              ...
              <CardBody>
                {!isLoading && prs && !isEditing && <Row></Row>}
                {isLoading && (
                  <Row>
