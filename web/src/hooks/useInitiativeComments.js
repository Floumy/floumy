...
function useInitiativeComments(initiative, setInitiative, toast) {
  ...
  const addComment = useCallback(
    async (comment) => {
      try {
        const addedComment = await addInitiativeComment(
          orgId,
          projectId,
          initiative.id,
          comment,
  ...
  const updateComment = useCallback(
    async (commentId, content) => {
      try {
        const updatedComment = await updateInitiativeComment(
          orgId,
          projectId,
          initiative.id,
          commentId,
        ...
        setInitiative((prevInitiative) => {
          const index = prevInitiative.comments.findIndex(
